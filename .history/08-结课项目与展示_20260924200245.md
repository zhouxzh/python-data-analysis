...
4. **先理解计划，再检查命令，再小步执行，最后验证结果。** 看不懂就停下来让 DSH 解释，仍不懂就查看课程说明或问 DSH。
...
第一节课不写正式分析代码，只做三件事：选题、数据加载、写清验收标准。
...
`data/08-final/penguins.csv` 作为第 1-3 节的参考项目数据；正式选题仍从 Week 1-7 数据中选择。
...
这节课把项目变成“可运行、可验证、可迭代”的脚本，重点是 DSH 审查、反方意见和迭代记录。
...
这节课不再写新分析，只做三件事：3 分钟展示、互评质疑、写复盘。
...
互评围绕三件事打分：问题是否清楚、结论是否有依据、局限是否诚实。同学和评审可能问：
...
1. 完成一个可复现的数据分析或建模项目，主数据来自 Week 1-7 的 `data/`（或者自己查找到感兴趣的合法获取的数据）。
