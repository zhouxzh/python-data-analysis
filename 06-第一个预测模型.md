...
> 参考脚本：`scripts/06-classification/` 下 5 个文件，与正文代码块一一对应。
...
建模不是“把准确率调高”。本周先建立三条硬规则。第一，`data/` 只读，代码只写入 `projects/<姓名>/`。第二，先切分训练/测试集，测试集绝不参与训练。第三，每个结论必须带评估指标和样本量，不能只说“准确率不错”。所有随机操作统一 `random_state=42`。
...
演示：读数据、核对标签含义、构造 X/y、切分、缩放、训练、打印准确率和混淆矩阵。练习：改 `test_size`、向 DSH 追问每个参数，并用测试集标签手工核对矩阵格子。复盘：按验收清单逐项确认。
...
脚本：[01-german-credit-audit.py](https://github.com/zhouxzh/python-data-analysis/blob/main/scripts/06-classification/01-german-credit-audit.py) ｜ 数据：[GermanCredit.csv](https://github.com/zhouxzh/python-data-analysis/blob/main/data/07-modeling/GermanCredit.csv)

看交叉表。`critical account/other credits existing` 表示坏账历史，293 人里有 243 人是 `credit_risk == 1`。`no credits taken` 表示无信贷记录，情况较好，40 人里只有 15 人是 `1`。因此本数据中 `1` 表示高风险，`0` 表示低风险。本节课把 `credit_risk == 1` 当作阳性。
...
脚本：[02-german-credit-logistic.py](https://github.com/zhouxzh/python-data-analysis/blob/main/scripts/06-classification/02-german-credit-logistic.py) ｜ 数据：[GermanCredit.csv](https://github.com/zhouxzh/python-data-analysis/blob/main/data/07-modeling/GermanCredit.csv)
...
演示：删除标识列、编码分类变量、跑“永远猜多数类”的基线，再对比默认模型与 `class_weight='balanced'` 模型。练习：手工从混淆矩阵计算 precision/recall，并向 DSH 追问为什么准确率高还会漏报。复盘：按验收清单逐项确认。
...
脚本：[03-churn-classification-compare.py](https://github.com/zhouxzh/python-data-analysis/blob/main/scripts/06-classification/03-churn-classification-compare.py) ｜ 数据：[Churn_Modelling.csv](https://github.com/zhouxzh/python-data-analysis/blob/main/data/07-modeling/Churn_Modelling.csv)
...
演示：用 `cross_val_score` 比较 Dummy、LogisticRegression、DecisionTree 三个模型的 F1 和 ROC AUC。练习：把混淆矩阵翻译成误报/漏报数量，并向 DSH 追问模型边界。复盘：按验收清单逐项确认。
...
脚本：[04-churn-model-comparison.py](https://github.com/zhouxzh/python-data-analysis/blob/main/scripts/06-classification/04-churn-model-comparison.py) ｜ 数据：[Churn_Modelling.csv](https://github.com/zhouxzh/python-data-analysis/blob/main/data/07-modeling/Churn_Modelling.csv)
...
### 3.9 扩展数据练习 Titanic 生存预测

第 3 节把同一套流程换到 `data/07-modeling/titanic.csv`。数据 891 行、15 列，目标是 `survived`。它比 Churn 更短，适合练习“删标识、编码、补缺失、基线、建模、报告”。
...
脚本：[05-titanic-classification.py](https://github.com/zhouxzh/python-data-analysis/blob/main/scripts/06-classification/05-titanic-classification.py) ｜ 数据：[titanic.csv](https://github.com/zhouxzh/python-data-analysis/blob/main/data/07-modeling/titanic.csv)
...
dummy accuracy: 0.6157
logistic accuracy: 0.7985
[[140  25]
 [ 29  74]]
              precision    recall  f1-score   support

           0     0.8284    0.8485    0.8383       165
           1     0.7475    0.7184    0.7327       103

    accuracy                         0.7985       268
   macro avg     0.7879    0.7835    0.7855       268
weighted avg     0.7973    0.7985    0.7977       268
```

读法：测试集 268 人，其中 165 人未生存、103 人生存。逻辑回归准确率 0.7985，明显高于只猜多数的 0.6157；混淆矩阵显示它抓到 74 个真生存者，漏掉 29 个。

练习：

1. 把 `Sex`、`Embarked` 换成 `pd.get_dummies` 编码，比较结果是否一致，并说明哪种方式更适合课堂讲解。
2. 删除 `Embarked` 后重跑，观察分类报告是否变化，解释缺失列为什么可能破坏复现。
3. 手工从混淆矩阵计算 precision 和 recall，与分类报告核对。
