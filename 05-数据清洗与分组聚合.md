...
`data/04-cleaning/Cars93_miss.csv` 是 93 行 × 27 列的汽车数据，字段含 `Manufacturer`、`Model`、`Type`、`Price`、`MPG.city`、`MPG.highway`、`AirBags`、`Cylinders`、`Weight`、`Origin`、`Make`。本节课只回答三个问题：
...
完整脚本是 `scripts/05-cleaning-merge/01-cars93-missing.py`：
...
实际输出（`results/05-cleaning-merge/01-cars93-missing-result.txt`）：
...
`data/04-cleaning/telco_customer_churn.csv` 是 4,225 行 × 52 列的电信客户数据，字段含 `Customer ID`、`Churn`、`Tenure in Months`、`Monthly Charge`、`Total Charges`、`Contract`、`Internet Type`、`Internet Service`。回答三个问题：
...
完整脚本是 `scripts/05-cleaning-merge/02-telco-churn.py`：
...
实际输出（`results/05-cleaning-merge/02-telco-churn-result.txt`）：
...
- `norway_new_car_sales_by_make.csv`：4,377 行 × 5 列，品牌级销量，字段 `Year`、`Month`、`Make`、`Quantity`、`Pct`。
- `norway_new_car_sales_by_model.csv`：2,694 行 × 6 列，车型级销量，多一个 `Model` 字段。

`data/06-merge/MarketArrivals.csv` 是 10,227 行 × 10 列的到货数据，字段含 `market`、`month`、`year`、`quantity`、`state`、`city`。回答三个问题：
...
`scripts/05-cleaning-merge/03-norway-merge.py`：
...
实际输出（`results/05-cleaning-merge/03-norway-merge-result.txt`）：
...
`scripts/05-cleaning-merge/04-market-arrivals.py`：
...
实际输出（`results/05-cleaning-merge/04-market-arrivals-result.txt`）：
...
- 透视表是 12 个月 × 21 年（1996–2016）。2016 年只有 January 和 February 有值，其余月份是 `NaN`，说明 2016 是部分年份数据，不是整年。
...
- [ ] 能用 `pivot_table` 得到 12 × 21 的月份乘年份矩阵
