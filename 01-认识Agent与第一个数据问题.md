...
1. **看不懂的命令不执行。** Agent 请求执行命令时，先读命令本身；看不懂就让它解释，解释完仍看不懂就查看课程说明或问 DSH。
2. **只在自己的工作区里干活。** 课程任务只允许修改 `projects/<姓名>/` 和课程指定的目录；原始数据目录只读。
...
| 安装不必要或危险的软件 | Agent 为了“解决眼前问题”直接装包 | 依赖由课程统一清单决定；新增依赖必须课程确认 |
| API Key 泄露 | 为了“方便调试”让 Agent 写进代码或笔记 | Key 只在 DSH 设置里保存；任何文件里出现 Key 都要警惕 |
| 下载不可信数据 | Agent 联网搜索并保存结果 | 先检查来源；课程数据以 `data/` 和课程提供的文件为准 |
...
如果 Anaconda 或 Node.js 下载超过 10 分钟，先改用课程预置的离线安装包，不要原地等待。
...
如果 DSH 回答的路径不是本课程仓库，先改回本仓库再继续。本课程的规则是：**DSH 只能修改 `projects/` 和课程明确指定的文件，原始 `data/` 目录只读。**

每次允许 DSH 执行命令前，按第 1 节课的安全流程检查：命令是否看得懂、会不会改原数据、会不会删除文件、是不是安装额外软件。看不懂就让 DSH 解释，解释后仍不懂就查看课程说明。
...
| Agent 请求执行看不懂的命令 | 学生直接点了“允许” | 让 Agent 逐字解释命令；解释后仍不懂就查看课程说明或问 DSH |
...
1. 用自己的话写 300 字以内的“Agent 使用安全说明”，至少包含：Agent 是什么、5 条安全规则、3 个绝对不执行的操作、为什么每条结论都要自己验证。保存到 `projects/<姓名>/agent-safety.md`。
